...
□ Audit Log قابل حذف/ویرایش از هیچ مسیری نیست — پوشش M2: `AccessMatrixTest` + `AuditLogTest`
□ کاربر غیرفعال (is_active=false) نمی‌تواند وارد شود — پوشش M2: `AccessMatrixTest` + `AuthenticationTest`
□ مسیرهای /api/v1/* بدون توکن 401 می‌دهند، نه 500/redirect — پوشش M2: `AccessMatrixTest` + `ApiAuthTest`
```
