...
**دلیل فنی (مستند در بخش ۷):** هیچ User Model، هیچ Token، هیچ Session امنیتی، هیچ OAuth/SSO و هیچ مکانیزم MFA در هیچ‌یک از سه سیستم و�یر، دلیل فنی را مستند کن.»
...
| **X-5** | **نسخهٔ PHP روی هاست متفاوت است** | هاست‌های اشتراکی ممکن است PHP 8.1/8.2 داشته باشند | حداقل نسخهٔ پشتیبانی‌شده **PHP 8.3** تعیین می‌شود؛ بررسی نسخه در CI و در یک Health Check |
...
| **D-1** | Stack فنی | **PHP 8.2+ + Laravel 12 + MySQL/MariaDB + Tailwind v4** *(اصلاح M2: پیاده‌سازی واقعی `PHP 8.3+ + Laravel 13` است — رجوع به `composer.json`؛ پیش‌فرض تأیید ضمنی — کاربر سؤال را رد کرد و دستور ادامه داد)* | ۱۴۰۵/۰۷/۰۲ | تنها گزینهٔ سازگار با XAMPP و مهارت PHP تیم · شروع Milestone 1 |
| **D-2** | لایهٔ UI | **Blade + Alpine.js** برای فاز بنیان *(پیش‌فرض)* | ۱۴۰۵/۰۷/۰۲ | بدون نیاز به Node در توسعه · ارتقای بعدی به Inertia/React بدون بازنویسی Backend |
| **D-4** | مقیاس هدف | **پیش‌فرض: کوچک تا متوسط** — زیر ۱۰۰ پرسنل · یک شرکت · Schema چندشعبه‌ای · Tenant Isolation غیرفعال *(پیش‌فرض — قابل تغییر)* | ۱۴۰۵/۰۷/۰۲ | ایندکس‌گذاری استاندارد · صفحه‌بندی همه‌جا · ستون‌های `company_id` آماده ولی بدون Global Scope |
| **P-1** | اولویت قدم بعدی | **Milestone 1 معماری HRM** — رفع آسیب‌پذیری‌های Management System در scope این مخزن نیست و در این سند مستند می‌ماند *(پیش‌فرض)* | ۱۴۰۵/۰۷/۰۲ | شروع پیاده‌سازی بنیان HRM |

> **نکته:** تصمیمات D-1 · D-2 · D-4 · P-1 به‌صورت پیش‌فرض (پیشنهاد معمار) ثبت شده‌اند چون کاربر سؤالات تأیید را رد کرد و دستور «ادامه» داد.
> هرکدام با یک پیام قابل برگشت هستند؛ تأثیر برگشت D-2 و D-4 کم، و D-1 زیاد است.

### ⬜ ۲۵.۲ تصمیمات باز

| # | تصمیم | وضعیت |
|---|---|---|
| — | همهٔ تصمیمات لازم برای Milestone 1 ثبت شده‌اند | ریسک‌های باقی‌مانده در بخش ۲۳ (U-2…U-14) ردیابی می‌شوند |
...
| ۱۴۰۵/۰۷/۰۲ | M1: معماری و Design System روی Laravel 13 + PHP 8.3 پیاده شد (این سند در M1 به‌روز نشد) |
| ۱۴۰۵/۰۷/۰۲ | M2: اصلاح کهنگی نسخه‌ها (Laravel 12←13، حداقل PHP 8.3) · یادداشت اصلاحیهٔ D-1 · ماژول Identity کامل شد |
