...
| **Database** | **MySQL 8.0 / MariaDB 10.6+** با `utf8mb4_persian_ci` | هم‌راستا با `config.php:8` که این Collation را توصیه کرده؛ در دسترس روی هاست اشتراکی； پشتیبانی از CTE بازگشتی (درخت سازمان) و JSON |
| **جایگزین DB** | PostgreSQL 16 (در صورت سرور اختصاصی) | بهتر برای داده‌های سلسله‌مراتبی و Full-Text فارسی |
...
| **Cache / Queue** | Redis (یا درایور `database` برای هاست اشتراکی) | MODULE 40 — اعلان‌ها، انقضای قرارداد، گزارش‌ها |
...
> **گزینهٔ کم‌ریسک جایگزین:** در صورت ترجیح تیم، **Laravel + Blade + Alpine.js + Tailwind** برای فاز بنیان.
> در هر دو حالت **لایهٔ Application و `/api/v1` یکسان** است، بنابراین تعویض UI بعداً بازنویسی Backend نمی‌طلبد.
> این تصمیم در بخش ۲۵ برای تأیید مطرح شده است.
...
| محیط | هدف | دیتابیس | ویژگی‌ها |
|---|---|---|---|
| **development** | توسعهٔ محلی | MySQL محلی / SQLite | debug روشن · دادهٔ Seed ساختگی · Mail/Queue روی `log`/`sync` |
| **staging** | پیش‌تولید | MySQL ایزوله | آینهٔ production · HTTPS · دادهٔ **ساختگی** · بررسی مهاجرت |
| **production** | واقعی | MySQL با پشتیبان | HTTPS · کوکی امن · لاگینگ · مانیتورینگ · پشتیبان رمزنگاری‌شده |
...
|---|---|---|---|
| U-1 | **زیرساخت استقرار** | سرور اختصاصی / VPS / هاست اشتراکی / XAMPP محلی؟ | تعیین‌کننده برای MySQL در برابر PostgreSQL، Redis در برابر درایور database، و امکان اجرای Queue Worker |
...
## ۲۵. تصمیمات مورد نیاز برای تأیید

پیش از آغاز پیاده‌سازی (Milestone 1)، تأیید شما در ۴ مورد ضروری است:

| # | تصمیم | گزینهٔ پیشنهادی من | جایگزین |
|---|---|---|---|
| **D-1** | **Stack فنی** | PHP 8.3 + Laravel 12 + MySQL 8 | Node/NestJS · Python/Django |
| **D-2** | **لایهٔ UI** | Inertia 2 + React 19 + TypeScript | Blade + Alpine (کم‌ریسک‌تر) · SPA کامل |
| **D-3** | **زیرساخت استقرار** (U-1) | ⬜ **نیازمند پاسخ شما** | VPS · هاست اشتراکی · لوکال/XAMPP |
| **D-4** | **مقیاس هدف** (U-2/U-3) | ⬜ **نیازمند پاسخ شما** | <۵۰ پرسنل · ۵۰–۵۰۰ · >۵۰۰ |

**سؤالات تکمیلی که پاسخ آن‌ها معماری را دقیق‌تر می‌کند (اختیاری اما مفید):**
